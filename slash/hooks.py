...
_define(
    "tests_loaded",
    doc="Called when Slash finishes loading a batch of tests for execution (not necessarily al tests)",
    arg_names=("tests",),
)

_define(
    "before_session_start",
    doc="Entry point which is called before session_start, useful for configuring plugins and other global resources",
)
_define(
    "after_session_start",
    doc="Second entry point for session start, useful for plugins relying on other plugins' session_start routine",
)
...
_define(
    "configure",
    doc="Configuration hook that happens during commandline parsing, and before plugins are activated. It is a convenient point to override plugin activation settings",
...
_define(
    "test_distributed",
    doc="Called in parallel mode, after the parent sent a test to child)",
    arg_names=(
        "test_logical_id",
        "worker_session_id",
    ),
...
_define(
    "log_file_closed",
    doc="Called right after a log file was closed",
    arg_names=(
        "path",
        "result",
    ),
...
_define(
    "error_added",
    doc="Called when an error is added to a result (either test result or global)",
    arg_names=("error", "result"),
)
_define(
    "interruption_added",
    doc="Called when an exception is encountered that triggers test or session interruption",
...
_define(
    "exception_caught_before_debugger",
...
_define(
    "exception_caught_after_debugger", doc="Called whenever an exception is caught, and a debugger has already been run"
)
_define(
    "before_worker_start",
    doc="Called in parallel execution mode, before the parent starts the child worker",
    arg_names=("worker_config",),
)

_define(
    "prepare_notification",
    doc="Called with a message object prior to it being sent via the notifications plugin (if enabled)",
    arg_names=("message",),
)
...
@_deprecated_to_gossip
def get_all_hooks():
    return [(hook.name, hook) for hook in gossip.get_group("slash").get_hooks()]
