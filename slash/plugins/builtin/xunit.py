...
from xml.etree.ElementTree import tostring as xml_to_string, Element as E, SubElement as SE
...
class Plugin(PluginInterface):
    ...
    def _get_test_case_element(self, test):
        return E(
            "testcase",
            dict(
                name=str(test), classname="{}.{}".format(test.__class__.__module__, test.__class__.__name__), time="0"
            ),
        )
    ...
    def session_end(self):
        ...
        suite_time = sum(
            [result.get_duration() for result in context.session.results.iter_test_results()], datetime.timedelta()
        ).total_seconds()
        ...
        e = E(
            "testsuite",
            {
                ...
                "skipped": str(context.session.results.get_num_skipped(include_not_run=False)),
            },
        )
        self._add_errors(e, context.session.results.global_result)
        run_test_results = filter(lambda result: result.is_started(), context.session.results.iter_test_results())

        for result in run_test_results:
            test = E(
                "testcase", {"name": result.test_metadata.address, "time": str(result.get_duration().total_seconds())}
            )
