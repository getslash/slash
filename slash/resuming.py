...
class ResumedTestData(object):
    ...
    def __repr__(self):
        return "<ResumedTestData({!r}, {!r}, {!r}, {!r})>".format(
            self.file_name, self.address_in_file, self.variation, self.status
        )

    def __eq__(self, other):
        return (
            self.file_name == other.file_name
            and self.address_in_file == other.address_in_file
...
def clean_old_entries():
    ...
    with connecting_to_db() as conn:
        ...
        if old_sessions_ids:
            conn.query(ResumeState).filter(ResumeState.session_id.in_(old_sessions_ids)).delete(
...
def save_resume_state(session_result, collected_tests):
    session_metadata = SessionMetadata(
        session_id=session_result.session.id, src_folder=os.path.abspath(os.getcwd()), created_at=datetime.now()
    )

    tests_to_resume = []
    for result in session_result.iter_test_results():
        metadata = result.test_metadata
        test_to_resume = ResumeState(
            session_id=session_result.session.id, file_name=metadata.file_path, address_in_file=metadata.address_in_file
        )
    ...
    for test_metadata in tests_with_no_results:
        test_to_resume = ResumeState(
            ...
            file_name=test_metadata.file_path,
            address_in_file=test_metadata.address_in_file,
            status=ResumeTestStatus.PLANNED,
        )
...
def get_tests_from_remote(session_id, get_successful_tests):
    ...
    kwargs = (
        {"filters_dict": {"show_successful": get_successful_tests}}
        if backslash_version >= (2, 38)
        else {"get_successful": get_successful_tests}
    )
