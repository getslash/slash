...
class _Cmdline(object):
    ...
    def configure_parser(self, parser, path, node):
        ...
        if self.append is not None:
            parser.add_argument(
                self.append, dest=self.arg_dest, action="append", metavar=self.metavar, default=None, help=description
            )

        self._add_arg(parser, self.on, callback=_set_true, description="Turn on " + description)
        self._add_arg(parser, self.off, callback=_set_false, description="Turn off " + description)
        self._add_arg(
            parser, self.increase, callback=_increase, description=self.increase_doc or ("Increase " + description)
        )
        self._add_arg(
            parser, self.decrease, callback=_decrease, description=self.decrease_doc or ("Decrease " + description)
        )
