...
def _notify_going_into_debugger(exc_info):
    if context.session is not None:
        ...
        print(
            "\nException caught in debugger: {}".format(
                traceback.format_exception_only(exc_info[0], exc_info[1])[0].strip()
            )
        )
...
def debug_if_needed(exc_info=None):
    ...
    if (
        context.session
        and isinstance(exc_info[1], context.session.get_skip_exception_types())
        and not config.root.debug.debug_skips
    ):
        return
