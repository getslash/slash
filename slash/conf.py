...
config = Config(
    {
        "debug": {
            ...
            "filter_strings": []
            // Doc("A string filter, selecting if to enter pdb")
            // Cmdline(append="--pdb-filter", metavar="FILTER"),
            "debugger": None,
        },
        "log": {
            ...
            "truncate_console_lines": True
            // Doc("truncate long log lines on the console")
            // Cmdline(arg="--truncate-console-lines", metavar="yes/no"),
            "truncate_console_errors": False
            // Doc(
            ...
            "compression": {
                ...
                "use_rotating_raw_file": False
                // Doc("When compression is enabled, write also to uncompressed rotating log file"),
            },
            "cleanup": {
                "enabled": False,
                "keep_failed": True,
        ...
        "run": {
            ...
            "repeat_each": 1
            // Doc("Repeat each test a specified amount of times")
            // Cmdline(arg="--repeat-each", metavar="NUM_TIMES"),
            "repeat_all": 1
            // Doc("Repeat all suite a specified amount of times")
            ...
            "message_assertion_introspection": True
            // Doc("When False, failing assertions which have messages attached will not emit introspection info"),
            "capture": {
        ...
        "parallel": {
            ...
            // Doc("Server address")
            // Cmdline(arg="--parallel-addr", metavar="PARALLEL_SERVER_ADDRESS"),
            "server_port": 0 // Doc("Server port") // Cmdline(arg="--parallel-port", metavar="PARALLEL_SERVER_PORT"),
            "keepalive_port": 0
            // Doc("Keepalive port")
            // Cmdline(arg="--keepalive-port", metavar="KEEPALIVE_SERVER_PORT"),
            "parent_session_id": None
            // Doc("parent session id")
            // Cmdline(arg="--parallel-parent-session-id", metavar="MASTER_SESSION_ID"),
            ...
            "workers_error_dir": None
            // Doc("workers error directory")
            // Cmdline(arg="--workers-error-dir", metavar="WORKERS_ERROR_DIR"),
        },
        "resume": {
            "failed_first": False
            // Doc("Run failed tests of previous session before all others")
            // Cmdline(on="--failed-first", metavar="FAILED_FIRST"),
            "unstarted_first": False
            // Doc("Run unstarted tests of previous session before all others")
            // Cmdline(on="--unstarted-first", metavar="UNSTARTED_FIRST"),
            "failed_only": False
            // Doc("Run only failed tests of previous session")
            // Cmdline(on="--failed-only", metavar="FAILED_ONLY"),
            "unstarted_only": False
            // Doc("Run only unstarted tests of previous session")
            // Cmdline(on="--unstarted-only", metavar="UNSTARTED_ONLY"),
            "no_skipped": False
            // Doc("Run tests of previous session without skipped tests")
            // Cmdline(on="--no-skipped", metavar="NO_SKIPPED"),
        ...
        "tmux": {
            ...
            "use_panes": False
            // Doc("In parallel mode, run children inside panes and not windows")
            // Cmdline(on="--tmux-panes"),
        },
        "sentry": {
            "dsn": None
            // Doc(
