...
class KeepaliveServer(object):
    ...
    def serve(self):
        server = xmlrpc_server.SimpleXMLRPCServer(
            (config.root.parallel.server_addr, 0), allow_none=True, logRequests=False
        )
        try:
...
class Server(object):
    def __init__(self, tests):
        ...
        self.collection = [
            [test.__slash__.file_path, test.__slash__.function_name, test.__slash__.variation.dump_variation_dict()]
    ...
    def report_client_failure(self, client_id):
        ...
        if test_index is not None:
            _logger.error(
                "Worker {} interrupted while executing test {}",
                client_id,
    ...
    def validate_collection(self, client_id, sorted_client_collection):
        ...
        if (
            self.num_collections_validated >= config.root.parallel.num_workers
            and self.state == ServerStates.WAIT_FOR_COLLECTION_VALIDATION
        ):
    ...
    def get_test(self, client_id):
        if not self.executing_tests[client_id] is None:
            _logger.error(
                "Client_id {} requested new test without sending former result", client_id, extra={"capture": False}
            )
        ...
        if self.state == ServerStates.STOP_TESTS_SERVING:
            ...
            hooks.test_distributed(
                test_logical_id=test.__slash__.id, worker_session_id=self._get_worker_session_id(client_id)
            )  # pylint: disable=no-member
            _logger.notice(
                "#{}: {}, Client_id: {}",
                test_index + 1,
                test.__slash__.address,
                client_id,
    ...
    def finished_test(self, client_id, result_dict):
        ...
        if test_index is not None:
            ...
            with _get_test_context(self.tests[test_index], logging=False) as (result, _):
                ...
                if result.has_fatal_exception() or (
                    not result.is_success(allow_skips=True) and config.root.run.stop_on_error
                ):
                    _logger.debug(
                        "Server stops serving tests, run.stop_on_error: {}, result.has_fatal_exception: {}",
                        config.root.run.stop_on_error,
                        result.has_fatal_exception(),
                    )
                    self.state = ServerStates.STOP_TESTS_SERVING
                    self._mark_unrun_tests()
        else:
            _logger.error(
                "finished_test request from client_id {} with index {}, but no test is mapped to this worker",
                client_id,
                test_index,
                extra={"capture": False},
            )
    ...
    def serve(self):
        server = xmlrpc_server.SimpleXMLRPCServer(
            (config.root.parallel.server_addr, config.root.parallel.server_port), allow_none=True, logRequests=False
        )
        try:
            self.port = server.server_address[1]
            self.state = ServerStates.WAIT_FOR_CLIENTS
