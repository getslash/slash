...
class WorkerConfiguration(object):
    def __init__(self, args, worker_id):
        ...
        self.argv = (
            [sys.executable, "-m", "slash.frontend.main", args.cmd, "--parallel-parent-session-id", ctx.session.id]
            + args.argv
            + ["--parallel-worker-id", str(worker_id)]
        )
    ...
    def start(self):
        ...
        self.argv += [
            "--parallel-port",
            str(ctx.session.parallel_manager.server.port),
            "--keepalive-port",
            str(ctx.session.parallel_manager.keepalive_server.port),
            "--workers-error-dir",
            ctx.session.parallel_manager.workers_error_dircetory,
        ]
        self._start()
    ...
    def _test_or_index_to_index(self, test_or_test_index):
        test_index = (
            test_or_test_index.__slash__.parallel_index
            if hasattr(test_or_test_index, "__slash__")
            else test_or_test_index
        )
    ...
    def exclude_test(self, test_or_test_index):
        ...
        if test_index in self._forced_tests:
            raise RuntimeError(
                "Cannot exclude test_index {} for client {} since it has been forced before".format(
                    test_index, self._worker_id
                )
            )
        if test_index in self._excluded_tests:
            _logger.warning(
                "test_index {} already in exclude list for worker {}, not adding it", test_index, self._worker_id
            )
        else:
    ...
    def force_test(self, test_or_test_index):
        ...
        if test_index in self._excluded_tests:
            raise RuntimeError(
                "Cannot force test_index {} for client {} since it has been excluded before".format(
                    test_index, self._worker_id
                )
            )
        if test_index in self._forced_tests:
            _logger.warning(
                "test_index {} already in forced list for worker {}, not adding it", test_index, self._worker_id
            )
        else:
