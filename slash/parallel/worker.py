...
class Worker(object):
    def __init__(self, client_id, session_id):
        ...
        self.keepalive_server_addr = "http://{}:{}".format(
            config.root.parallel.server_addr, config.root.parallel.keepalive_port
        )
    ...
    def warning_added(self, warning):
        try:
            ...
            _logger.error(
                "Failed to pickle warning. Message: {}, File: {}, Line: {}",
                warning.message,
                warning.filename,
                warning.lineno,
                extra={"capture": False},
    ...
    def start_execution(self, app, collected_tests):
        if _HAS_PID_PGID:
            ...
            _logger.debug("Skipping setsid for process since it is not available")
        ...
        if not self.client.validate_collection(self.client_id, sorted(collection)):
            _logger.error(
                "Collections of worker id {} and master don't match, worker terminates",
                self.client_id,
                extra={"capture": False},
            )
            ...
            try:
                while not should_stop:
                    ...
                    if test_entry == WAITING_FOR_CLIENTS:
                        ...
                        _logger.error(
                            "Worker_id {} recieved protocol error message, terminating",
                            self.client_id,
                            extra={"capture": False},
                        )
                        break
