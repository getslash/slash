...
class Result(object):
    ...
    def deserialize(self, result_dict):
        for key, value in result_dict.items():
            try:
                ...
                _logger.error(
                    "Error when deserialize reult, skipping this value. key = {}", key, extra={"capture": False}
                )
        ...
        if isinstance(exc_value, exceptions.FAILURE_EXCEPTION_TYPES):
            ...
            if not self.is_global_result():
                ...
                if not interrupted_test and not context.session.has_children():
                    with notify_if_slow_context(
                        message="Cleaning up test due to interrupt. Please wait..."
    ...
    def add_failure(self, e=None, frame_correction=0, exc_info=None, append=True):
        """Adds a failure to the result"""
        err = self._add_error(
            self._failures, e, frame_correction=frame_correction + 1, exc_info=exc_info, is_failure=True, append=append
        )
    ...
    def has_fatal_errors(self):
        return any(e.is_fatal() for e in itertools.chain(self._errors, self._failures))
...
class GlobalResult(Result):
    ...
    def is_success(self, allow_skips=False):
        ...
        if self._session_results is None:
            return True
        if (
            self._session_results.session.has_children()
            and self._session_results.session.parallel_manager.server.worker_error_reported
        ):
            return False
...
class SessionResults(object):
    ...
    def __repr__(self):
        return "<Results: {} successful, {} errors, {} failures, {} skips>".format(
            self.get_num_successful(), self.get_num_errors(), self.get_num_failures(), self.get_num_skipped()
        )
    ...
    def create_result(self, test):
        if test.__slash__.id in self._results_dict:
            raise exceptions.SlashInternalError(
                "{} shouldn't appear in results dict before adding its result".format(test.__slash__.id)
            )
