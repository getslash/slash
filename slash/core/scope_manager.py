...
class ScopeManager(object):
    ...
    def _pop_scope(self, scope):
        ...
        call_all_raise_first([self._session.cleanups.pop_scope, self._session.fixture_store.pop_scope], scope)
        _logger.trace("Popped scope {0}", popped)

    def flush_remaining_scopes(self):
        _logger.trace("Flushing remaining scopes: {}", self._scopes)
        call_all_raise_first([functools.partial(self._pop_scope, s) for s in self._scopes[::-1]])
