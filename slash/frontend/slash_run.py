...
from ..resuming import (
    get_last_resumeable_session_id,
    get_tests_from_previous_session,
    save_resume_state,
    clean_old_entries,
)
...
def slash_run(args, report_stream=None, resume=False, rerun=False, app_callback=None, working_directory=None):
    ...
    if resume:
        ...
        app.arg_parser.set_description(
            "Resumes a previously run session by running only unsuccessful on unfinished tests"
        )
    else:
                ...
                with handling_exceptions():
                    ...
                    if resume or rerun:
                        ...
                        to_resume = [
                            ...
                            for x in get_tests_from_previous_session(
                                session_id, get_successful_tests=get_successful_tests
                            )
                        ]
                        collected = app.test_loader.get_runnables(
                            to_resume, prepend_interactive=app.parsed_args.interactive
                        )
                    else:
