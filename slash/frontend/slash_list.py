    ...
    try:
        ...
        with slash.Session() as session:
            ...
            runnables = loader.get_runnables(
                itertools.chain(parsed_args.paths, iter_suite_file_paths(parsed_args.suite_files))
            )
            used_fixtures = set()
...
def _report_fixtures(args, session, printer, used_fixtures):
    ...
    for fixture in session.fixture_store:
        ...
        printer("    Source: {}:{}".format(source_path, inspect.getsourcelines(fixture_func)[1]))
        printer("\n")
