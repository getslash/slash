...
def validate_run(suite, run_result, expect_interruption, expect_session_errors):
    if expect_interruption:
        assert (
            run_result.session.results.global_result.is_interrupted()
...
def _validate_single_test(test, results):
    ...
    for param_values in _iter_param_value_sets(test):
        is_excluded = any(
            (param_names[param_id], value) in test.excluded_param_values for param_id, value in param_values.items()
        )
        ...
        for repetition in range(test.get_num_expected_repetitions()):  # pylint: disable=unused-variable
            for index, result in enumerate(results):
                ...
                assert False, "Could not find parameter set {}".format(param_values)
...
def _validate_single_test_result(test, result):
    ...
    if expected == "ERROR":
        ...
        raise NotImplementedError("Unknown expected result: {!r}".format(expected))  # pragma: no cover
