...
def details(request, result):
    ...
    if request.param == "complex":
        result.details.append("detail1", "value1")
        result.details.append("detail1", {"complex": ["value2", "value3"]})
...
def validate_xml(xml_filename, suite=None, results=None):
    ...
    if results is not None:
        run_tests_results = list(
            filter(lambda result: result.is_started(), results.session.results.iter_test_results())
        )
    ...
    for child in list(root):
        ...
        for subchild in iter(child):
            ...
            test_case_tag_count.update({subchild.tag: test_case_tag_count[subchild.tag] + 1})
