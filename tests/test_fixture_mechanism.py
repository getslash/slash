...
def _get_all_values(store, fixture_name):
    returned = []
    for variation in store.iter_parametrization_variations(
        fixture_ids=[store.get_fixture_by_name(fixture_name).info.id]
    ):
...
def test_nested_store_resolution_activation(store):
    ...
    assert store.get_fixture_dict(["fixture2"]) == {"fixture2": "2"}
...
def test_fixture_store_add(fixture_func, fixture_func_name):
    ...
    assert f.get_fixture_by_name(fixture_func_name).fixture_func is fixture_func
