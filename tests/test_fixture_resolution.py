...
def test_resolve_fixture_object_namespace_correctness():
    with slash.Session() as s:
        ...
        test = tests[0]
        resolved = store.resolve_name(
            "local_fixture.global_fixture_1.dependency_fixture", test, namespace=test.get_fixture_namespace()
        ).info
...
def test_resolve_fixture_parameterization_with_scopes(suite_builder):
    ...
    suite_builder.build().run().assert_success(8).with_data(
        [{"params": x} for x in list(itertools.product(range(1, 3), range(3, 5), range(5, 7)))]
    )
