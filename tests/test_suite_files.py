...
def test_iter_suite_paths_files_relpath(filename, paths):
    ...
    assert [os.path.abspath(p) for p, _ in suite_files.iter_suite_file_paths([filename])] == [
...
@pytest.mark.parametrize("string", ["some_path.py #", "some_path.py # bbb", "some_path.py#"])
...
def paths(request, tmpdir, use_relpath_for_dir):  # pylint: disable=redefined-outer-name
    ...
    basenames.extend(["file100.py:SomeClass", "file101.py:Someclass.test_method"])
