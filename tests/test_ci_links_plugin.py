...
BUILD_URLS = ("https://jenkins/job/some-job/123/", "https://different-jenkins/another-test/9999/")
ENV_VAR_NAMES = ("MY_BUILD_URL_VARIABLE", "ANOTHER_VARIABLE")
LOG_PATH_TEMPLATES = ("%(build_url)s/%(log_path)s", "https://my-log-storage-server/%(log_path)s")
...
def test_build_url_defined(suite, build_url):
    ...
    result = summary.get_all_results_for_test(test)[0]
    log_link = result.details.all().get("log_link")
    expected_log_link = "/".join((build_url.rstrip("/"), "artifact", *result.get_log_path().split(os.sep)))
...
def test_nondefault_build_url_environment_variable(suite, config_override, build_url_env_var):
    env_var_name, env_var_value = build_url_env_var
    manager.install(Plugin(), is_internal=False)
    config_override("plugin_config.ci_links.build_url_environment_variable", env_var_name)
    manager.activate("ci links")
    test = suite.add_test()
    test.when_run.fail()
    summary = suite.run()
    result = summary.get_all_results_for_test(test)[0]
    log_link = result.details.all().get("log_link")
    expected_log_link = "/".join((env_var_value.rstrip("/"), "artifact", *result.get_log_path().split(os.sep)))
    assert log_link == expected_log_link


@pytest.mark.parametrize("template", LOG_PATH_TEMPLATES)
def test_nondefault_link_template(suite, config_override, build_url, template):
    manager.install(Plugin(), is_internal=False)
    ...
    expected_log_link = template % {"build_url": build_url.rstrip("/"), "log_path": local_log_path}
    assert log_link == expected_log_link
