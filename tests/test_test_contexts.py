        ...
        class Test1(slash.Test):
            ...
            def test_1(self):
                assert _recorder["Context1.before_case(Test1.test_1)"].timestamp < \
                    _recorder[
                        "Context2.before_case(Test1.test_1)"].timestamp, "Context1 did not happen before Context2"
        ...
        class Test2(slash.Test):
            ...
            def test_1(self):
                ...
                assert _recorder.events[
                    "Context2.before_case(Test2.test_1)"].happened

        run_tests_assert_success(itertools.chain(Test1.generate_tests('', ''),
        ...
        with slash.Session() as session:
            with session.get_started_context():
                slash.runner.run_tests(
                    itertools.chain(Test1.generate_tests('', ''), Test2.generate_tests('', '')))

        [result1, result2] = session.results.iter_test_results()
        self.assertTrue(result1.is_skip())
        self.assertTrue(result2.is_skip(),
                        "{0} is not skipped".format(result2))
