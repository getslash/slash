...
def test_hook__error_added_during_test(suite, request, checkpoint, suite_test):
    request.addfinalizer(
        hooks.error_added.register(checkpoint).unregister  # pylint: disable=no-member
    )
...
def test_hook__test_interrupt(suite, request, checkpoint):
    request.addfinalizer(
        hooks.test_interrupt.register(checkpoint).unregister  # pylint: disable=no-member
    )
...
def test_hook__test_failure_without_exception(suite, request, checkpoint, suite_test):
    request.addfinalizer(
        hooks.test_failure.register(checkpoint).unregister  # pylint: disable=no-member
    )
...
def test_debugger_called_on_hooks(
    request, hook_name, exception_type, should_raise, forge, config_override, checkpoint, debug_enabled
):
...
class HookCallingTest(_TestCase):
    def setUp(self):
        ...
        self.plugin2 = make_custom_plugin(
            "plugin2", self, hook_names=["before_session_start", "session_start", "after_session_start"]
        )
