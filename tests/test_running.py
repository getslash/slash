...
def test_debug_if_needed(request, config_override, suite, suite_test):
    ...
    request.addfinalizer(
        functools.partial(setattr, slash.utils.debug, "_KNOWN_DEBUGGERS", slash.utils.debug._KNOWN_DEBUGGERS)
    )
...
def fatal_error_adder(request, failure_type):
    def adder(test):
        if request.param == "raising":
            test.append_line("from slash.exception_handling import mark_exception_fatal")
            test.append_line(
                "raise mark_exception_fatal({}())".format(
                    "AssertionError" if failure_type == "failure" else "Exception"
                )
            )
