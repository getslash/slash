...
def test_logs_compression(
    files_dir, suite, config_override, compression_enabled, compression_method, use_rotating_raw_file
):
...
class LogFormattingTest(TestCase):
    def setUp(self):
        ...
        self.override_config("log.root", self.log_path)
        self.override_config("log.format", "-- {record.message} --")
...
class LoggingTest(TestCase):
    def test(self):
        ...
        self.override_config(
            "log.subpath",
            os.path.join(
                "{context.session.id}", "{context.test.__slash__.test_index0:03}-{context.test_id}", "debug.log"
            ),
        )
        self.override_config("log.session_subpath", os.path.join("{context.session.id}", "debug.log"))
        self.override_config("log.silence_loggers", [_silenced_logger.name])
        ...
        self.session = run_tests_assert_success(SampleTest)
        self.tests_metadata = [result.test_metadata for result in self.session.results.iter_test_results()]
        self._test_all_run()
    ...
    def _test_all_run(self):
        methods = [method_name for method_name in dir(SampleTest) if method_name.startswith("test")]
    ...
    def _test_test_logs_written(self):
        for test_metadata in self.tests_metadata:
            ...
            log_path = os.path.join(self.log_path, self.session.id, test_dir, "debug.log")
        ...
        for path, _, filenames in os.walk(self.log_path):
            for filename in filenames:
                ...
                with open(filename) as f:
                    assert _silenced_logger.name not in f.read(), "Silenced logs appear in log file {}".format(filename)
...
class TestLocaltimeLogging(TestCase):
    def setUp(self):
        ...
        self.override_config("log.localtime", True)
        self.override_config("log.root", self.path)
